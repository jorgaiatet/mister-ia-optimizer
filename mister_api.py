...
def fetch_squad_and_saldo(token: str, community_id: int, team_id: int) -> dict:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            return {"success": False, "error": f"Error obteniendo plantilla (HTTP {res.status_code})"}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def fetch_market_players(token: str, community_id: int) -> dict:
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            return {"success": False, "error": f"Error obteniendo mercado (HTTP {res.status_code})"}
    except Exception as e:
        return {"success": False, "error": str(e)}
